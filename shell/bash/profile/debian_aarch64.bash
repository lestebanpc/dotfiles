...
# Ruta del folder donde se ubican comandos personalizado del usuario.
if [ "$_g_command_path" != "/usr/local/bin" ] && [ "$_g_command_path" != "$HOME/.local/bin" ]; then
    PATH="${_g_command_path}:${PATH}"
fi

# GraalVM - RTE y Herramientas de desarrollo para Java y otros
if [ -d "${_g_program_base_path}/graalvm" ]; then
    GRAALVM_HOME="${_g_program_base_path}/graalvm"
    export GRAALVM_HOME
...
[ -d "${_g_program_base_path}/neovim/bin" ] && PATH="$PATH:${_g_program_base_path}/neovim/bin"

# CMake - Sistema de contrucción para C/C++ y otros
[ -d "${_g_program_base_path}/cmake" ] && PATH="$PATH:${_g_program_base_path}/cmake/bin"

# Go - Tools estandar para desarrollo
[ -d "${_g_program_base_path}/go/bin" ] && PATH="$PATH:${_g_program_base_path}/go/bin"

# Go - Tools adicionales
[ -d ~/go/bin ] && PATH="$PATH:${HOME}/go/bin"

# Rust - Tools para desarrollo
[ -d "${_g_program_base_path}/rust/bin" ] && PATH="$PATH:${_g_program_base_path}/rust/bin"
#[ -d ~/.cargo/bin ] && PATH="$PATH:${HOME}/.cargo/bin"

# LLVM/Clang/Clangd
if [ -d "${_g_program_base_path}/llvm/bin" ]; then
    PATH="${_g_program_base_path}/llvm/bin:$PATH"
elif [ -d "${_g_program_base_path}/lsp_servers/clangd/bin" ]; then
    PATH="${_g_program_base_path}/lsp_servers/clangd/bin:$PATH"
fi

# Ruta del builder Apache Maven
[ -d "${_g_program_base_path}/maven/bin" ] && PATH="${_g_program_base_path}/maven/bin:$PATH"

# Ruta del compilador de ProtoBuffer de gRPC
[ -d "${_g_program_base_path}/protoc/bin" ] && PATH="${_g_program_base_path}/protoc/bin:$PATH"

# Node.Js (RTE)
[ -d "${_g_program_base_path}/nodejs/bin" ] && PATH="${_g_program_base_path}/nodejs/bin:$PATH"

# DotNet
if [ -d "${_g_program_base_path}/dotnet" ]; then
    export DOTNET_ROOT="${_g_program_base_path}/dotnet"
...
# AWS CLI v2
[ -d "${_g_program_base_path}/aws-cli" ] && PATH="${_g_program_base_path}/aws-cli:$PATH"
#[ -d "${_g_program_base_path}/aws-cli/v2/current/bin" ] && PATH="${_g_program_base_path}/aws-cli/v2/current/bin:$PATH"
...
#[ -d "${_g_program_base_path}/cni_plugins" ] && export CNI_PATH=${_g_program_base_path}/cni_plugins
...
eval "$(oh-my-posh --init --shell bash --config ${_g_ohmyposh_theme_path})"
