...
function _get_current_path_base() {
    ...
    l_path=${l_path%/.files/shell/bash/setup/linux/*}
    echo "$l_path"
    return 0
}
...
. ${g_path_base}/.files/shell/bash/shared/utility_general.bash
...
. ${g_path_base}/.files/shell/bash/setup/linux/_common_utility.bash
...
. ${g_path_base}/.files/shell/bash/setup/linux/_setup_commands_custom_settings.bash

#Personalización: Funciones modificables para el instalador.
. ${g_path_base}/.files/shell/bash/setup/linux/_setup_commands_custom_logic.bash
    ...
    for((l_j=0; l_j < ${l_n}; l_j++)); do
        ...
        if [ $l_status_first_setup -ne -1 ]; then
            ...
            #Si se ejecuta usando el menú
                ...
                printf -v l_title_template "%sGroup (%s) >%s Repository %s(%s/%s)%s > '%s%s%s' %s%%s%s" "$g_color_gray1" "$l_option_value" "$g_color_reset" "$g_color_gray1" \
...
g_usage() {
    ...
    printf '    %b~/.files/shell/bash/setup/linux/01_setup_commands.bash uninstall\n%b' "$g_color_yellow1" "$g_color_reset"
    printf '    %b~/.files/shell/bash/setup/linux/01_setup_commands.bash uninstall PRG_PATH CMD_BASE_PATH TEMP_PATH\n%b' "$g_color_yellow1" "$g_color_reset"
    printf '  > %bInstalar repositorios mostrando el menú de opciones (interactivo)%b:\n' "$g_color_cian1" "$g_color_reset"
    printf '    %b~/.files/shell/bash/setup/linux/01_setup_commands.bash\n%b' "$g_color_yellow1" "$g_color_reset"
    printf '    %b~/.files/shell/bash/setup/linux/01_setup_commands.bash 0\n%b' "$g_color_yellow1" "$g_color_reset"
    printf '    %b~/.files/shell/bash/setup/linux/01_setup_commands.bash 0 PRG_PATH CMD_BASE_PATH TEMP_PATH SETUP_ONLYLAST_VERSION\n%b' "$g_color_yellow1" "$g_color_reset"
    printf '  > %bInstalar/Actualizar un grupo de repositorios sin mostrar el menú%b:\n' "$g_color_cian1" "$g_color_reset"
    printf '    %b~/.files/shell/bash/setup/linux/01_setup_commands.bash CALLING_TYPE MENU-OPTIONS\n%b' "$g_color_yellow1" "$g_color_reset"
    printf '    %b~/.files/shell/bash/setup/linux/01_setup_commands.bash CALLING_TYPE MENU-OPTIONS PRG_PATH CMD_BASE_PATH TEMP_PATH SUDO-STORAGE-OPTIONS\n%b' "$g_color_yellow1" "$g_color_reset"
    printf '    %b~/.files/shell/bash/setup/linux/01_setup_commands.bash CALLING_TYPE MENU-OPTIONS PRG_PATH CMD_BASE_PATH TEMP_PATH SUDO-STORAGE-OPTIONS SETUP_ONLYLAST_VERSION OTHER-USERID\n%b' "$g_color_yellow1" "$g_color_reset"
    printf '    %bDonde:%b\n' "$g_color_gray1" "$g_color_reset"
    ...
    printf '    %b~/.files/shell/bash/setup/linux/01_setup_commands.bash CALLING_TYPE LIST-REPO-ID%b\n' "$g_color_yellow1" "$g_color_reset"
    printf '    %b~/.files/shell/bash/setup/linux/01_setup_commands.bash CALLING_TYPE LIST-REPO-ID PRG_PATH CMD_BASE_PATH TEMP_PATH SUDO-STORAGE-OPTIONS%b\n' "$g_color_yellow1" "$g_color_reset"
    printf '    %b~/.files/shell/bash/setup/linux/01_setup_commands.bash CALLING_TYPE LIST-REPO-ID PRG_PATH CMD_BASE_PATH TEMP_PATH SUDO-STORAGE-OPTIONS SETUP_ONLYLAST_VERSION SHOW-TITLE-1REPO OTHER-USERID%b\n' "$g_color_yellow1" "$g_color_reset"
    printf '    %bDonde:%b\n' "$g_color_gray1" "$g_color_reset"
    ...
    printf '  > %bOTHER-USERID %bEl UID/GID del usuario al que es owner del script (el repositorio git) en formato "UID:GID". Solo si se ejecuta como root y este es diferente al onwer del script. Si se envia vacio o EMPTY no se considera.%b\n\n' \
           "$g_color_green1" "$g_color_gray1" "$g_color_reset"
...
#Folder donde se almacena los binarios. Su valor es autogenerado por "set_command_path" y puede ser:  
# - "${g_path_cmd_base}/bin"
# - "/usr/local/bin"
# - "~/.local/bin" 
g_path_bin=''

#Folder donde se almacena los archivos de ayuda man1. Su valor es autogenerado por "set_command_path" y puede ser:  
# - "${g_path_cmd_base}/man/man1"
# - "/usr/local/man/man1"
# - "~/.local/man/man1" 
g_path_man=''

#Folder donde se almacena los archivos fuentes. Su valor es autogenerado por "set_command_path" y puede ser:  
# - "${g_path_cmd_base}/share/fonts"
# - "/usr/share/fonts"
# - "~/.local/share/fonts" 
g_path_fonts=''

#Si es WSL, obtener las carpetas usadas para almacenar los programas y comandos en Windows.
if [ $g_os_type -eq 1 ]; then

    #Si no es folder valido (no existe y no tiene permisos de escritura).
    if [ -z "$g_path_base_win" ] || [ ! -w "$g_path_base_win" ]; then
        if [ -d "/mnt/d" ]; then
            g_path_base_win='/mnt/d/CLI'
        else
            g_path_base_win='/mnt/c/CLI'
        fi
    fi

    g_path_programs_win="${g_path_base_win}/Programs"
    g_path_bin_win="${g_path_base_win}/Commands/bin"
    g_path_man_win="${g_path_base_win}/Commands/man"
    g_path_doc_win="${g_path_base_win}/Commands/doc"
    g_path_etc_win="${g_path_base_win}/Commands/etc"
fi
...
if [ $gp_uninstall -eq 0 ]; then
    ...
    #Obtener la ruta real del folder de comandos 'g_path_bin', archivos de ayuda 'g_path_man' y fuentes de letras 'g_path_fonts' 
    if [ ! -z "$3" ] && [ "$3" != "EMPTY" ]; then
        #La prioridad siempre es el valor enviado como argumento, luego el valor del archivo de configuración '_config.bash'
        g_path_cmd_base="$3"
    fi
    ...
    #Obtener la ruta real del folder temporal 'g_path_temp'
    if [ ! -z "$4" ] && [ "$4" != "EMPTY" ]; then
        #La prioridad siempre es el valor enviado como argumento, luego el valor del archivo de configuración '_config.bash'
        g_path_temp="$4"
    fi
...
#2.2. Instalar y actualizar los artefactos de un repositorio
    ...
    if [ $gp_type_calling -eq 0 ]; then
        ...
        #Obtener la ruta real del folder de archivos temporales 'g_path_temp'
        if [ ! -z "$4" ] && [ "$4" != "EMPTY" ]; then
            #La prioridad siempre es el valor enviado como argumento, luego el valor del archivo de configuración '_config.bash'
            g_path_temp="$4"
        fi
    ...
    #2.2.2. Instalando los repositorios especificados por las opciones indicas en '$2'
        ...
        #Si se ejecuta un usuario root y es diferente al usuario que pertenece este script de instalación (es decir donde esta el repositorio)
        g_other_calling_user=''
        if [ $g_user_sudo_support -eq 4 ] && [ ! -z "$8" ] && [ "$8" != "EMPTY" ] && [ "$g_path_base" != "$HOME" ]; then
            if [[ "$8" =~ ^[0-9]+:[0-9]+$ ]]; then
        ...
        #Obtener la ruta real del folder de los archivos temporales 'g_path_temp'
        if [ ! -z "$5" ] && [ "$5" != "EMPTY" ]; then
            #La prioridad siempre es el valor enviado como argumento, luego el valor del archivo de configuración '_config.bash'
            g_path_temp="$5"
        fi
    ...
    #2.2.3. Instalando un solo repositorio del ID indicao por '$2'
        ...
        #Si se ejecuta un usuario root y es diferente al usuario que pertenece este script de instalación (es decir donde esta el repositorio)
        g_other_calling_user=''
        if [ $g_user_sudo_support -eq 4 ] && [ ! -z "$9" ] && [ "$9" != "EMPTY" ] && [ "$g_path_base" != "$HOME" ]; then
            if [[ "$9" =~ ^[0-9]+:[0-9]+$ ]]; then
                g_other_calling_user="$9"
            else
                echo "Parametro 9 \"$9\" debe ser tener el formado 'UID:GID'."
                exit 110
            fi
        fi
