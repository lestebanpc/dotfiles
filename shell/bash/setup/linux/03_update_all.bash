...
#Nota: Los script de instalación tiene una ruta similar a 'PATH_BASE/REPO_NAME/shell/bash/setup/linux/SCRIPT.bash', donde 'REPO_NAME' siempre es '.files'.
#
...
function _copy_plugin_files() {
    ...
    case "$p_repo_name" in
        ...
        fzf)
            ...
            #cp "${p_repo_path}/shell/completion.bash" "${g_path_base}/.files/shell/bash/autocomplete/fzf.bash"
            ...
            #cp "${p_repo_path}/shell/key-bindings.bash" "${g_path_base}/.files/shell/bash/keybindings/fzf.bash"
            #
            ## Script que se usara como comando para abrir fzf en un panel popup tmux
            #echo "Copiando el script \"./bin/fzf-tmux\" como \"~/.files/shell/bash/fzf/fzf-tmux.bash\" y crear un enlace el como comando \"~/.local/bin/bash/fzf-tmux\"..."
            #cp "${p_repo_path}/bin/fzf-tmux" "${g_path_base}/.files/shell/bash/fzf/fzf-tmux.bash"
...
function _update_all() {
    ...
    if [ $l_flag -eq $l_opcion ]; then
        ...
        if [ $l_is_noninteractive -eq 1 ]; then
            ${g_path_base}/.files/shell/bash/setup/linux/01_setup_commands.bash 1 2 "$g_path_programs" "" "$g_path_temp" $g_status_crendential_storage 1 "$g_other_calling_user"
            l_status=$?
        else
            ${g_path_base}/.files/shell/bash/setup/linux/01_setup_commands.bash 3 2 "$g_path_programs" "" "$g_path_temp" $g_status_crendential_storage 1 "$g_other_calling_user"
            l_status=$?
        fi
...
g_usage() {
    ...
    printf '    %b~/.files/shell/bash/setup/linux/03_update_all.bash\n%b' "$g_color_yellow1" "$g_color_reset"
    printf '    %b~/.files/shell/bash/setup/linux/03_update_all.bash 0 PRG_PATH CMD_BASE_PATH TEMP_PATH SETUP_ONLYLAST_VERSION\n%b' "$g_color_yellow1" "$g_color_reset"
    printf '  > %bActualizaciones SIN usar un menú de opciones:%b:\n' "$g_color_cian1" "$g_color_reset"
    printf '    %b~/.files/shell/bash/setup/linux/03_update_all.bash CALLING_TYPE MENU-OPTIONS PRG_PATH CMD_BASE_PATH TEMP_PATH\n%b' "$g_color_yellow1" "$g_color_reset"
    printf '    %b~/.files/shell/bash/setup/linux/03_update_all.bash CALLING_TYPE MENU-OPTIONS PRG_PATH CMD_BASE_PATH TEMP_PATH SUDO-STORAGE-OPTIONS SETUP_ONLYLAST_VERSION OTHER-USERID\n\n%b' "$g_color_yellow1" "$g_color_reset"
    printf 'Donde:\n'
