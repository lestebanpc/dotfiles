...
function g_install_options() {
    ...
    if [ $p_input_options -gt 0 ] && [ $(( $p_input_options & $l_option )) -eq $l_option ] && [ ! -z "$p_list_pckg_ids" ]; then
        ...
        if [ $g_user_sudo_support -ne 2 ] && [ $g_user_sudo_support -ne 3 ]; then
            ...
            if [ $l_is_noninteractive -eq 1 ]; then
                ${g_path_base}/.files/shell/bash/setup/linux/04_setup_packages.bash 2 "$p_list_pckg_ids" $g_status_crendential_storage $p_flag_upgrade_os_pkgs
                l_status=$?
            else
                ${g_path_base}/.files/shell/bash/setup/linux/04_setup_packages.bash 4 "$p_list_pckg_ids" $g_status_crendential_storage $p_flag_upgrade_os_pkgs
                l_status=$?
            fi
    ...
    if [ $p_input_options -gt 0 ] && [ $(( $p_input_options & $l_option )) -eq $l_option ]; then
        ...
        if [ $g_user_sudo_support -ne 2 ] && [ $g_user_sudo_support -ne 3 ]; then
            ...
            # 7> Install only last version: por defecto es 1 (representa a 'false'). Solo si su valor es 0 representa a 'true'.
            # 8> El GID y UID del usuario que ejecuta el script, siempre que no se el owner de repositorio, en formato "UID:GID".
            if [ $l_is_noninteractive -eq 1 ]; then
                ${g_path_base}/.files/shell/bash/setup/linux/01_setup_commands.bash 1 4 "$g_path_programs" "$g_path_cmd_base" "$g_path_temp" $g_status_crendential_storage \
                    0 "$g_other_calling_user"
                l_status=$?
            else
                ${g_path_base}/.files/shell/bash/setup/linux/01_setup_commands.bash 3 4 "$g_path_programs" "$g_path_cmd_base" "$g_path_temp" $g_status_crendential_storage \
                    0 "$g_other_calling_user"
                l_status=$?
            fi
    ...
    if [ ! -z "$p_list_repo_ids" ]; then
        ...
        if [ $l_is_noninteractive -eq 1 ]; then
            ...
            ${g_path_base}/.files/shell/bash/setup/linux/01_setup_commands.bash 2 "$p_list_repo_ids" "$g_path_programs" "$g_path_cmd_base" "$g_path_temp" \
                $g_status_crendential_storage 0 1 "$g_other_calling_user"
            l_status=$?
        else
            ${g_path_base}/.files/shell/bash/setup/linux/01_setup_commands.bash 4 "$p_list_repo_ids" "$g_path_programs" "$g_path_cmd_base" "$g_path_temp" \
                $g_status_crendential_storage 0 1 "$g_other_calling_user" 
            l_status=$?
        fi
    ...
    if [ $l_prg_options -ne 0 ] && [ $l_prg_options -ne 4 ] && [ $l_prg_options -ne 5 ]; then
       ...
       if [ $l_is_noninteractive -eq 1 ]; then
           ${g_path_base}/.files/shell/bash/setup/linux/02_setup_profile.bash 1 $l_prg_options "$g_path_programs" "$g_path_temp" $g_status_crendential_storage "$g_other_calling_user"
           l_status=$?
       else
           ${g_path_base}/.files/shell/bash/setup/linux/02_setup_profile.bash 2 $l_prg_options "$g_path_programs" "$g_path_temp" $g_status_crendential_storage "$g_other_calling_user"
           l_status=$?
       fi
...
g_usage() {
    ...
    printf '    %b~/.files/shell/bash/setup/linux/00_setup_summary.bash uninstall\n%b' "$g_color_yellow1" "$g_color_reset"
    printf '  > %bInstalar repositorios mostrando el menú de opciones (interactivo)%b:\n' "$g_color_cian1" "$g_color_reset"
    printf '    %b~/.files/shell/bash/setup/linux/00_setup_summary.bash\n%b' "$g_color_yellow1" "$g_color_reset"
    printf '    %b~/.files/shell/bash/setup/linux/00_setup_summary.bash 0 LIST-PCKG-IDS PRG_PATH CMD_BASE_PATH TEMP_PATH CLEAN-OS-CACHE\n%b' "$g_color_yellow1" "$g_color_reset"
    printf '  > %bInstalar/Actualizar un grupo de opciones sin mostrar el menú%b:\n' "$g_color_cian1" "$g_color_reset"
    printf '    %b~/.files/shell/bash/setup/linux/00_setup_summary.bash CALLING_TYPE MENU-OPTIONS\n%b' "$g_color_yellow1" "$g_color_reset"
    printf '    %b~/.files/shell/bash/setup/linux/00_setup_summary.bash CALLING_TYPE MENU-OPTIONS LIST-REPO-IDS LIST-PCKG-IDS PRG_PATH CMD_BASE_PATH TEMP_PATH\n%b' "$g_color_yellow1" "$g_color_reset"
    printf '    %b~/.files/shell/bash/setup/linux/00_setup_summary.bash CALLING_TYPE MENU-OPTIONS LIST-REPO-IDS LIST-PCKG-IDS PRG_PATH CMD_BASE_PATH TEMP_PATH SUDO-STORAGE-OPTIONS CLEAN-OS-CACHE UPGRADE-OS-PACKAGES OTHER-USERID\n\n%b' \
...
#2.2. Instalando los repositorios especificados por las opciones indicas en '$2'
    ...
    #Si se ejecuta un usuario root y es diferente al usuario que pertenece este script de instalación (es decir donde esta el repositorio)
    g_other_calling_user=''
    if [ $g_user_sudo_support -eq 4 ] && [ ! -z "${11}" ] && [ "${11}" != "EMPTY" ] && [ "$g_path_base" != "$HOME" ]; then
        if [[ "${11}" =~ ^[0-9]+:[0-9]+$ ]]; then
            g_other_calling_user="${11}"
        else
            echo "Parametro 11 \"$8\" debe ser tener el formado 'UID:GID'."
            exit 110
        fi
    fi
