...
_install_nodejs() {
    ...
    if [ $l_is_noninteractive -eq 1 ]; then
        ${g_path_base}/.files/shell/bash/setup/linux/01_setup_commands.bash 2 "nodejs" "$g_path_programs" "" "$g_path_temp" $g_status_crendential_storage 1 1 "$g_other_calling_user"
        l_status=$?
    else
        ${g_path_base}/.files/shell/bash/setup/linux/01_setup_commands.bash 4 "nodejs" "$g_path_programs" "" "$g_path_temp" $g_status_crendential_storage 1 1 "$g_other_calling_user"
        l_status=$?
    fi
...
_install_python() {
    ...
    if [ $l_is_noninteractive -eq 1 ]; then
        ${g_path_base}/.files/shell/bash/setup/linux/04_setup_packages.bash 2 "$l_packages_to_install" $g_status_crendential_storage
        l_status=$?
    else
        ${g_path_base}/.files/shell/bash/setup/linux/04_setup_packages.bash 4 "$l_packages_to_install" $g_status_crendential_storage
        l_status=$?
    fi
...
function _install_vim() {
    ...
    # 3> El estado de la credencial almacenada para el sudo
    if [ $l_is_noninteractive -eq 1 ]; then
        ${g_path_base}/.files/shell/bash/setup/linux/04_setup_packages.bash 2 'vim' $g_status_crendential_storage
        l_status=$?
    else
        ${g_path_base}/.files/shell/bash/setup/linux/04_setup_packages.bash 4 'vim' $g_status_crendential_storage
        l_status=$?
    fi
...
function _install_nvim() {
    ...
    if [ "$g_os_architecture_type" = "aarch64" ] || [ $g_os_subtype_id -eq 1 ]; then
        ...
        # 3> El estado de la credencial almacenada para el sudo
        if [ $l_is_noninteractive -eq 1 ]; then
            ${g_path_base}/.files/shell/bash/setup/linux/04_setup_packages.bash 2 "nvim" $g_status_crendential_storage
            l_status=$?
        else
            ${g_path_base}/.files/shell/bash/setup/linux/04_setup_packages.bash 4 "nvim" $g_status_crendential_storage
            l_status=$?
        fi
    ...
    #Actualmente (2023), en github solo existe binarios para x64 y no para arm64
        ...
        if [ $l_is_noninteractive -eq 1 ]; then
            ...
            ${g_path_base}/.files/shell/bash/setup/linux/01_setup_commands.bash 2 "neovim" "$g_path_programs" "" "$g_path_temp" $g_status_crendential_storage 1 1 "$g_other_calling_user"
            l_status=$?
        else
            ${g_path_base}/.files/shell/bash/setup/linux/01_setup_commands.bash 4 "neovim" "$g_path_programs" "" "$g_path_temp" $g_status_crendential_storage 1 1 "$g_other_calling_user"
            l_status=$?
        fi
...
function _sutup_support_x11_clipboard() {
    ...
    # 3> El estado de la credencial almacenada para el sudo
    if [ $l_is_noninteractive -eq 1 ]; then
        ${g_path_base}/.files/shell/bash/setup/linux/04_setup_packages.bash 2 "$l_pkg_options" $g_status_crendential_storage
        l_status=$?
    else
        ${g_path_base}/.files/shell/bash/setup/linux/04_setup_packages.bash 4 "$l_pkg_options" $g_status_crendential_storage
        l_status=$?
    fi
...
function _setup_user_profile() {
    ...
    local l_source_path="${g_path_base}/.files/keys/tls"
    ...
    l_source_path="${g_path_base}/.files/tmux"
    ...
    l_source_path="${g_path_base}/.files/etc/nerdctl"
    ...
    if [ $g_user_is_root -ne 0 ] || [ ! -z "$g_other_calling_user" ]; then
        ...
        l_source_path="${g_path_base}/.files/etc/podman"
        ...
        #l_source_path="${g_path_base}/.files/etc/containerd"
        ...
        l_source_path="${g_path_base}/.files/etc/buildkit"
        l_source_filename='config_default.toml'
        _create_file_link "$l_source_path" "$l_source_filename" "$l_target_link" "Profile > " $l_flag_overwrite_ln
        l_status=$?
        #if [ $l_status -ne 0 ] && [ ! -z "$g_other_calling_user" ]; then
...
g_usage() {
    ...
    printf '    %b~/.files/shell/bash/setup/linux/02_setup_profile.bash\n%b' "$g_color_yellow1" "$g_color_reset"
    printf '    %b~/.files/shell/bash/setup/linux/02_setup_profile.bash 0 PRG_PATH CMD_BASE_PATH TEMP_PATH\n%b' "$g_color_yellow1" "$g_color_reset"
    printf '  > %bConfigurando el profile del usuario/VIM/NeoVIM segun un grupo de opciones de menú indicados%b:\n' "$g_color_cian1" "$g_color_reset"
    printf '    %b~/.files/shell/bash/setup/linux/02_setup_profile.bash CALLING_TYPE MENU-OPTIONS PRG_PATH CMD_BASE_PATH TEMP_PATH\n%b' "$g_color_yellow1" "$g_color_reset"
    printf '    %b~/.files/shell/bash/setup/linux/02_setup_profile.bash CALLING_TYPE MENU-OPTIONS PRG_PATH CMD_BASE_PATH TEMP_PATH SUDO-STORAGE-OPTIONS OTHER-USERID\n\n%b' "$g_color_yellow1" "$g_color_reset"
    printf 'Donde:\n'
