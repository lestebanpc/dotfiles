...
declare -A gA_subcmd_ids=(
        ['sshagent']='Permite que WSL2 se conecte al agente SSH de Windows'
    )
...
# -------------------------------------------------------------------------------------
# Subcomand Controller> SSH Agent
# -------------------------------------------------------------------------------------
...
main() {
    ...
    if ! command -v socat >/dev/null 2>&1; then
        printf '[%bERROR%b] Se debe tener el comando "%b%s%b" instalado.\n' "$g_color_red1" "$g_color_reset" "$g_color_gray1" "socat" "$g_color_reset"
        return 1
    fi
