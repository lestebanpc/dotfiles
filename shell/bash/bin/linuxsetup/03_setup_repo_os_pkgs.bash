...
# shellcheck source=/home/lucianoepc/.files/shell/bash/bin/linuxsetup/lib/setup_repo_os_pkgs_custom.bash
. ${g_shell_path}/bash/bin/linuxsetup/lib/setup_repo_os_pkgs_custom.bash
...
function _uninstall_menu_options() {
    ...
    if [ -z "$l_result" ]; then
        ...
        if [ $l_n -gt 1 ]; then
            ...
            if [ $gp_type_calling -eq 0 ]; then
                printf -v l_title_template "Package Group (%s%s%s) > '%s%s%s'" "$g_color_gray1" "$l_option_value" "$g_color_reset" "$g_color_cian1" "${ga_menuoption_title[${p_option_relative_idx}]}" "$g_color_reset"
            #Si se ejecuta sin usar el menu
            else
                printf -v l_title_template "Package Group > '%s%s%s'" "$g_color_cian1" "${ga_menuoption_title[${p_option_relative_idx}]}" "$g_color_reset"
            fi
...
g_usage() {
    ...
    printf '    %b%s/bash/bin/linuxsetup/03_setup_repo_os_pkgs.bash uninstall\n%b' "$g_color_yellow1" "$g_shell_path" "$g_color_reset"
    printf '    %b%s/bash/bin/linuxsetup/03_setup_repo_os_pkgs.bash uninstall REPO_NAME\n%b' "$g_color_yellow1" "$g_shell_path" "$g_color_reset"
    printf '  > %bInstalar paquetes mostrando el menú de opciones (interactivo)%b:\n' "$g_color_cian1" "$g_color_reset"
    printf '    %b%s/bash/bin/linuxsetup/03_setup_repo_os_pkgs.bash\n%b' "$g_color_yellow1" "$g_shell_path" "$g_color_reset"
    printf '    %b%s/bash/bin/linuxsetup/03_setup_repo_os_pkgs.bash 0 REPO_NAME\n%b' "$g_color_yellow1" "$g_shell_path" "$g_color_reset"
    printf '  > %bInstalar/Actualizar un grupo de paquetes sin mostrar el menú%b:\n' "$g_color_cian1" "$g_color_reset"
    printf '    %b%s/bash/bin/linuxsetup/03_setup_repo_os_pkgs.bash CALLING_TYPE MENU-OPTIONS REPO_NAME\n%b' "$g_color_yellow1" "$g_shell_path" "$g_color_reset"
    printf '    %b%s/bash/bin/linuxsetup/03_setup_repo_os_pkgs.bash CALLING_TYPE MENU-OPTIONS REPO_NAME SUDO-STORAGE-OPTIONS\n%b' "$g_color_yellow1" \
    ...
    printf '    %b%s/bash/bin/linuxsetup/03_setup_repo_os_pkgs.bash CALLING_TYPE LIST-REPO-IDS REPO_NAME%b\n' "$g_color_yellow1" "$g_shell_path" "$g_color_reset"
    printf '    %b%s/bash/bin/linuxsetup/03_setup_repo_os_pkgs.bash CALLING_TYPE LIST-REPO-IDS REPO_NAME SUDO-STORAGE-OPTIONS%b\n' "$g_color_yellow1" \
           "$g_shell_path" "$g_color_reset"
    printf '    %b%s/bash/bin/linuxsetup/03_setup_repo_os_pkgs.bash CALLING_TYPE LIST-REPO-IDS REPO_NAME SUDO-STORAGE-OPTIONS UPGRADE-OS-PACKAGES%b\n' \
           "$g_color_yellow1" "$g_shell_path" "$g_color_reset"
    printf '    %bDonde:%b\n' "$g_color_gray1" "$g_color_reset"
